...
#include <string>
#include <iomanip>
#include <bits/stdc++.h> // converte hex
...
int main() {
    ...
    cout << "Registers:" << endl;
    cout << "AC " << setfill('0') << setw(2) << converteHex(acumulador) << endl;
    cout << "PC " << setfill('0') << setw(2) << converteHex(programcounter) << endl;
    cout << endl << "Memory:" << endl;

    for (int i = 128; i < indice + 1; i++) {
        if (memoria[i] != comp[i])
            cout << converteHex(i) << " " << setfill('0') << setw(2) << converteHex(memoria[i]) << endl;
    }
